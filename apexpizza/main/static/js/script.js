...
function plusik(e){
    ...
    var currentVal = parseInt(document.getElementById("quantity " + e.parentNode.id.replace("cart ", "")).value)

    // If is not undefined
    if (!isNaN(currentVal)) {
        // Increment
        document.getElementById("quantity " + e.parentNode.id.replace("cart ","")).value = currentVal + 1
        var data = "plus " + e.parentNode.id.replace("cart ", "")
        change_amount(data, false)
        
    } else {
        // Otherwise put a 0 there
        document.getElementById("quantity " + e.parentNode.id.replace("cart ","")).value = 0
...
function minusik(e){
    ...
    if (!isNaN(currentVal) && currentVal > 0) {
        // Decrement one
        document.getElementById("quantity " + e.parentNode.id.replace("cart ","")).value = currentVal - 1
        var data = "minus " + e.parentNode.id.replace("cart ", "")
        change_amount(data, false)
